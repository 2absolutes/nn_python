...
def vector_matrix_addition(vector, matrix, debug=0):
    """
    ...
    if vector.shape[0] == 1 and vector.shape[1] == 1:
        # For a vector with just one element
        return scalar_matrix_addition(vector[0, 0], matrix)
...
def scalar_matrix_multiplication(scalar, matrix, debug=0):
    """
...
def matrix_subtraction(matrix1, matrix2, debug=0):
    """
        Addition of 2 2d matrices
        :param matrix1:
        :param matrix2:
...
def matrix_element_division(matrix1, matrix2, debug=0):
    """
    ...
    matrix_element_divide = ker_code.get_function("matrix_element_division")

    matrix_element_divide(matrix1_gpu, matrix2_gpu, output_matrix_gpu,
                          np.int32(matrix1.shape[0]), np.int32(matrix1.shape[1]),
                          np.int32(debug),
                          block=BLOCK_DIMS,
                          grid=GRID_DIMS)
